// trying to make the bullets fire from the players position
// trying to track bullet position to player so it can fire from player
// wouldnt allow me to change player position
// game keeps crashing after changing button imput to change screen

//  when trying to add
//if (key == ' ')
  //  shoot = true;
 // bulletList.add(new Bullet(player.x,player.y)); this part gives me a hassle
 // i tried restructing my code especially around player but with my limited knowledge i dont know to fix it right away
 
 // notes of shoot 
// enemies spawn too close to player at start
// some enemies get stcuk on the wall 
// some notes of lag sometimes when starting
//bullets need to disappear when hitting a enemy to make the game more challenging
// make the enemies bounch off eachother 
// add coins but not after changing starting position of enemies dont need people to waste money 
